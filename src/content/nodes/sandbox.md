...
flow:
  scenario: >-
    An agent asked to clean up a repository runs a delete that was not quite
    the delete anyone meant.
  path:
    - actor: You
      where: a person, not a system
      does: >-
        ask an agent to tidy up a repository
    - actor: A command
      where: wherever the product runs
      does: >-
        generated by the model, about to run somewhere
    ...
    - node: sandbox
      ...
      does: >-
        bounds where it runs, so a bad step damages a copy
      self: true
    - node: command-execution
      where: a bounded environment
      does: >-
        runs inside those bounds and returns the output
  returns: >-
    Blast radius, decided before the mistake rather than after
relations:
  - type: consumed-by
